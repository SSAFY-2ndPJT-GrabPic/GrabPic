...
public class EncyclopediaController {
    ...
    public ResponseEntity<List<InfoPreviewDTO>> searchEncyclopedia(@PathVariable long userId, @RequestParam(required = false) String ordo, @RequestParam(required = false) String familia, @RequestParam(required = false) String genus, @RequestParam(required = false) String species, @RequestParam int page, @RequestParam int limit ){
        try{
            return ResponseEntity.status(HttpStatus.OK).body(encyclopediaService.searchEncyclopedia(userId, ordo, familia, genus, species, page, limit));
        }catch (Exception e){
            log.error(e. getMessage());
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
