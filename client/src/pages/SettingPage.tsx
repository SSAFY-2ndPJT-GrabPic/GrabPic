...
export default function Setting() {
  ...
  return (
    <div className="flex flex-col">
      ...
      {!userInfo.provider && (
        <S.SettingBtn onClick={moveUserInfo}>
          <img src={modifyIconUrl} className="mr-8" />
          회원정보 수정
        </S.SettingBtn>
      )}
      <S.SettingBtn onClick={() => settingClick(1)}>
        <img src={logoutIconUrl} className="mr-8" />
        로그아웃
      </S.SettingBtn>
      <S.SettingBtn onClick={() => settingClick(2)}>
        <img src={secessionIconUrl} className="mr-8" />
        회원탈퇴
      </S.SettingBtn>
    </div>
  );
